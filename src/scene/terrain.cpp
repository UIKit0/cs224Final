...
//                std::cout<<"shifting to "<<i<<"from "<<(i - sx)<<std::endl;
                tiles[i][j] = tiles[i - sx][j];
            }
            for (int i = endx; dirx*(i - (endx - sx)) > 0; i -= dirx){
//                std::cout<<"nulling "<<i<<std::endl;
                tiles[i][j] = NULL;
...
//                std::cout<<"shifting to "<<j<<"from "<<(j - sy)<<std::endl;
                tiles[i][j] = tiles[i][j - sy];
            }
            for (int j = endy; diry*(j - (endy - sy)) > 0; j -= diry){
//                std::cout<<"nulling "<<j<<std::endl;
                tiles[i][j] = NULL;
                updateQueue.append(Update(i, j));
            }
        }
    ...
    while (!updateQueue.isEmpty()){
        ...
        for (int x = 0; x < TILE_SIZE; x++){
            for (int y = 0; y <  TILE_SIZE; y++){
                ...
                tiles[u.ix][u.iy]->terrain[x][y] = 4.0f*glm::perlin(loc*NOISE_COORDINATE_RATIO);
            }
        }
